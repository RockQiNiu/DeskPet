...
PetController::PetController(QObject *parent) : QObject(parent) {
    connect(&m_eventBus, &EventBus::eventPublished, this, [this](const PetEvent &event) {
        m_machine.dispatch(event.type);
    });
...
void PetController::trigger(QString name) {
    static const QHash<QString, PetEventType> map{{"idle",PetEventType::CpuNormal},{"walk",PetEventType::IdleTimeout},{"run",PetEventType::MouseLeaving},{"jump",PetEventType::JumpRequested},{"wave",PetEventType::WaveRequested},{"sit",PetEventType::PerchRequested},{"coding",PetEventType::UserCoding},{"success",PetEventType::BuildSuccess},{"failed",PetEventType::BuildFailed},{"cpu",PetEventType::HighCpu},{"commit",PetEventType::GitCommit},{"sleep",PetEventType::LateNight},{"crash",PetEventType::ProgramCrash},{"perch",PetEventType::PerchRequested},{"chase",PetEventType::MouseLeaving},{"avoid",PetEventType::MouseApproaching}};
    ...
    m_eventBus.publish({map.value(name)});
...
void PetController::completeAnimation(const QString &animation)
{
    if (animation == "jump" || animation == "wave" || animation == "celebrate")
        trigger("idle");
}
